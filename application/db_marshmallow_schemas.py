...
def validate_regions_unique(value: list):
    if len(value) != len(set(value)):
        raise ValidationError(f"{value} -> Регионы должны быть уникальны!")
...
def validate_unique_id(data, property_name: str):
    ...
    for record in data["data"]:
        if record[property_name] in record_ids:
            raise ValidationError(
                f"{property_name}: {record[property_name]} -> Идентификатор дублируется в данной выборке!"
            )
        record_ids.add(record[property_name])


class PatchCourierSchema(Schema):
    courier_type = EnumField(
        CourierType, by_value=True, error=ERROR_VALIDATION["courier_type"]
    )
    regions = List(
        Int(validate=Range(min=0), strict=True),
        validate=[
            Length(min=1, error=ERROR_VALIDATION["empty"]),
            validate_regions_unique,
        ],
    )
    working_hours = List(
        Str(validate=Regexp(TIME_PATTERN, error=ERROR_VALIDATION["time"])),
        validate=[
            Length(min=1, error=ERROR_VALIDATION["empty"]),
...
class OrderSchema(Schema):
    ...
    weight = MarshmallowFloat(validate=Range(min=0.01, max=50), required=True)
