...
from application.db_marshmallow_schemas import (
    CourierSchema,
    OrderSchema,
    ImportCourierSchema,
    ImportOrderSchema,
    ImportOrdersAssignSchema,
    ImportOrderCompleteSchema,
    PatchCourierSchema,
)
from application.db_models import db, Courier, Order
...
def courier_assigned_orders(current_session, courier_id: int):
    query = (
        ...
        .filter(and_(Order.courier_id == courier_id, Order.status == "assigned"))
        .order_by(Order.weight)
    )
    return query


def is_intervals_intersect(
    courier_time_intervals: list, order_time_intervals: list
) -> bool:
...
def check_pk_violation(current_session, id_name: str):
    ...
    try:
        ...
        raise ValidationError(
            f"{id_name}: {non_unique_courier_id}: -> Идентификатор уже существует!"
        )
...
def do_record_exists(record_id: int, property_name: str, table, current_session):
    try:
        ...
        if record is None:
            raise ValidationError(
                f"{property_name}: {record_id} -> Записи с данным ID не существует!"
            )
        return record
...
@api.route("/couriers/<int:courier_id>", methods=["PATCH"])
...
def patch_courier(loaded_request_data, courier_id):
    ...
    # обработка происходит в request_schema
    courier = do_record_exists(
        record_id=courier_id,
    ...
    if is_already_assigned:
        ...
        for order in assigned_orders:
            if not (
                ((backpack_weight + order.weight) <= courier_max_weight)
                and is_intervals_intersect(courier.working_hours, order.delivery_hours)
...
@api.route("/orders/assign", methods=["POST"])
@request_schema(ImportOrdersAssignSchema)
def assign_order(loaded_request_data):
    successful_response = {"orders": []}

    courier = do_record_exists(
        record_id=loaded_request_data["courier_id"],
        property_name="courier_id",
        table=Courier,
        current_session=db.session,
    )
    ...
    is_already_assigned = db.session.query(assigned_orders.exists()).scalar()
    if is_already_assigned:
        successful_response["orders"].extend(
            [{"id": order.order_id} for order in assigned_orders]
        )
        successful_response.update(
            {"assign_time": f"{dt_correct_output(assigned_orders[0].assign_time)}"}
        )
        return jsonify(successful_response), 200
    ...
    available_orders = (
        ...
        .filter(and_(Order.region.in_(courier.regions), Order.status == "available"))
        .order_by(Order.weight)
    )
    ...
    if len(matching_orders) == 0:
        ...
        successful_response["orders"].extend(
            [{"id": order} for order in matching_orders]
        )
        successful_response.update(
            {"assign_time": f"{dt_correct_output(assign_time_)}"}
        )
        return jsonify(successful_response), 200


@api.route("/orders/complete", methods=["POST"])
@request_schema(ImportOrderCompleteSchema)
def complete_order(loaded_request_data):
    # т.к. передается строка с <Z> на конце,
    # то приходится скармливать marshmallow это значение как aware datetime
    ...
    order = do_record_exists(
        record_id=loaded_request_data["order_id"],
        property_name="order_id",
        table=Order,
        current_session=db.session,
    )

    # Все подобные исключения перехватываются в request_schema
...
@api.route("/couriers/<int:courier_id>", methods=["GET"])
def get_courier_stat(courier_id):
    ...
    courier_completed_orders = db.session.query(Order).filter(
        and_(Order.courier_id == courier.courier_id, Order.status == "completed")
    )
    ...
    for order in courier_completed_orders:
        if order.region not in delivery_times_by_region.keys():
            ...
            delivery_times_by_region[order.region].extend(
                [order.assign_time, order.complete_time]
            )
        else:
    ...
    # Расчет earnings
    n = (
        db.session.query(Order.assign_time)
        .filter(Order.courier_id == courier.courier_id)
        .group_by(Order.assign_time)
        .count()
    )
    ...
    if db.session.query(courier_completed_orders.exists()).scalar():
        delivery_times_by_region_in_seconds = {
            region: [] for region in delivery_times_by_region.keys()
        }
        for region, delivery_times in delivery_times_by_region.items():
            i = 0
            while i < len(delivery_times) - 1:
                delivery_times_by_region_in_seconds[region].append(
                    abs((delivery_times[i] - delivery_times[i + 1]).total_seconds())
                )
                i += 1
        ...
        t = min(
            [
                mean(delivery_times)
                for delivery_times in delivery_times_by_region_in_seconds.values()
            ]
        )
